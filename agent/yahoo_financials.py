...
def _row_value(frame: Any, aliases: tuple[str, ...]) -> float | None:
    if _is_empty(frame):
        return None
    try:
        index_map = {str(label).strip().lower(): label for label in frame.index}
...
def _latest_annual(security: yf.Ticker) -> dict:
    try:
        ...
        income = None
    try:
        cashflow = security.get_cashflow(freq="yearly")
    except Exception:
        cashflow = None
    return _period_payload(income=income, cashflow=cashflow, basis="ANNUAL")


def _ttm(security: yf.Ticker) -> dict:
    try:
        income = security.get_income_stmt(freq="trailing")
    except Exception:
        try:
            ...
            income = None
    try:
        cashflow = security.get_cashflow(freq="trailing")
    except Exception:
        try:
            cashflow = security.ttm_cash_flow
        except Exception:
            cashflow = None
    return _period_payload(income=income, cashflow=cashflow, basis="TTM")
...
def fetch_yahoo_financial_snapshot(ticker: str) -> dict:
    ...
    return {
        ...
        "financial_period": (
            f"TTM through {period_end}" if basis == "TTM" and period_end else period_end
        ),
        "annual": annual,
        "ttm": ttm,
        "selected": selected,
        ...
        "notes": (
            ...
            "Free cash flow is deterministically recomputed as operating cash flow minus absolute capital expenditure."
        ),
    }
